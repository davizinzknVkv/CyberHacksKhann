...
document.querySelector("link[rel~='icon']").href = 'https://media.discordapp.net/attachments/1422721585642471505/1461148764948529203/static-removebg-preview.png?ex=69698054&is=69682ed4&hm=6694805329342022a11c36994556ca35bdf3f11e4383d5656bd69d73df60680e&format=webp&quality=lossless&';
...
async function initializeRepoPath() {
    for (const cdn of availableCDNs) {
        try {
            const response = await fetch(cdn + 'CyberHacksKhann.js', { method: 'HEAD' });
            if (response.ok) { 
                repoPath = cdn;
...
/* Visual Functions */
function setupMenu() {
    plppdo.on("domChanged",(()=>{if(document.getElementById("CyberHacksKhannTab"))return;const e=document.querySelector('nav[data-testid="side-nav"]');e&&(KWSection=document.createElement("section"),KWSection.id="CyberHacksKhannTab",KWSection.className="_evg4u4",KWSection.innerHTML='<h2 class="_n0asy6j">CyberHacksKhann</h2>',e.appendChild(KWSection))}));
